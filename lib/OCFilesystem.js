...
class OCFilesystem {
    ...
    /**
     * Gets space used on managed disks
     */
    spaceUsed() {
    ...
    /**
     * Gets read only mode on managed disks
     */
    isReadOnly() {
    ...
    open(file, mode = 'r') {
        ...
        if (this.exists(file) && (mode === "w" || mode === "w+" || mode === "a" || mode === "a+")) {
    ...
    read(file, count, offset) {
        ...
        offset -= 1;//Adjusting for javascript 0-index and lua 1-index.
