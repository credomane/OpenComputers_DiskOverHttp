--Author Lyra at https://oc.cil.li/topic/553-titan-bios-http-booting/
--License: Unknown
(function()
    local targets = {}
    ...
    for address in cl("screen", true) do
        if #ci(address, "getKeyboards") > 0 then
            screen = address
        end
    end
    ...
    for target in string.gmatch(ci(eeprom, "getData"), "([^,]+)") do
    ...
    local function boot_target(target)
        ...
        if string.match(target, "http://") or string.match(target, "https://") then
            ...
            if not req then
                error("Failed to issue request: " .. reason, 0)
            end
            while true do
        ...
        elseif string.match(target, "disk://") then
            ...
            for i, a in ipairs(filesystems) do
                if string.sub(a, 1, #disk) == disk then
                    address = a
                end
            end
            if not address then
                error("Can't find disk: " .. disk, 0)
            end
    ...
    if enable_menu then
        ...
        while true do
            ...
            if last_error then
                ci(gpu, "set", 1, #targets + 2, "Last error: " .. last_error)
            end
