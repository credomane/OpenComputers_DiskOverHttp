...
local componentCallback = {
    __call = function(self, ...)
        return proxylist[self.address][self.name](...)
    end,
    __tostring = function(self)
        return (doclist[self.address] ~= nil and doclist[self.address][self.name] ~= nil) and doclist[self.address][self.name] or "function"
    end
}

function component.virtual_register(address, ctype, proxy, doc)
    checkArg(1, address, "string")
...
function component.virtual_unregister(address)
    checkArg(1, address, "string")
    if proxylist[address] == nil then
...
function component.virtual_list()
    local list = {}
...
function component.virtual_resolve(address, componentType)
    checkArg(1, address, "string")
...
local r = math.random
function component.virtual_uuid()
